...
class AliyunOSSClosedAudioStorage(AliyunOSSStorageBase):
    """
    专用于存储音频文件的存储类
    """
    def __init__(self):
        super().__init__(
            bucket_name=settings.OSS_CLOSED_AUDIO_BUCKET_NAME,
            base_url=settings.OSS_CLOSED_AUDIO_BASE_URL
        )


class AliyunOSSClosedImagesStorage(AliyunOSSStorageBase):
    """
    专用于存储图片文件的存储类
    """
    def __init__(self):
        super().__init__(
            bucket_name=settings.OSS_CLOSED_IMAGES_BUCKET_NAME,
            base_url=settings.OSS_CLOSED_IMAGES_BASE_URL
        )